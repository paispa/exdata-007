...
png("C:\\A511400\\Git-Hub\\datasciencecoursera\\exData-007\\plot4.png")
par(mfrow = c(2, 2))
...
plot(strptime(paste(data$Date,data$Time),format="%d/%m/%Y %H:%M:%S"),data$volt,type="l",xlab="datetime",ylab="Voltage")
...
legend("topright",c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),lty = 1,col=c("black","red","blue"),box.col="white",inset = .02)
plot(strptime(paste(data$Date,data$Time),format="%d/%m/%Y %H:%M:%S"),data$reactive_pwr,type="l",xlab="datetime",ylab="Global_reactive_power")
dev.off()
